...
#         Server(TransbotProgramConfig, self.Transbot_Program_Reconfig)
    ...
    def img_setup(self, Function):
        # 图像美化玩法设置 Function: ["other"] Function: ["other"]  Image beautification gameplay setting Function: ["other"
        self.Function = Function
    ...
    def visual_process(self, action, rgb_img):
        # param action: [113 or 'q':退出quit]，[114 or 'r':重置reset]，[105 or 'i'：识别]，[32：开始追踪start tracker]
        binary = rgb_img
        self.action = action
...
if __name__ == '__main__':
    ...
    #     # transbot_program.AR_setup('Stickman')  # 第一步，有先后顺序
    #     transbot_program.visual_Tracker_setup("Follow_Line")  # 第一步，有先后顺序
    #     frame, binary = transbot_program.visual_process(action,frame)  # 第二步，有先后顺序
    #     frame = transbot_program.img_process(frame)  # 第二步，有先后顺序
    #     end = time.time()
    #     fps = 1 / (end - start)
