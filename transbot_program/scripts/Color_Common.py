...
def ManyImgs(scale, imgarray):
    ...
    #Determine whether the type of imgarray[0] is list，It is a list, indicating that imgarray is a tuple and needs to be displayed vertically
    ...
    if rowsAvailable:
        ...
        hor = [imgBlank] * rows  # 与第一张图片大小相同，与元组包含列表数相同的水平空白图像
                                 # The same size as the first image, and the same number of horizontal blank images as the tuple contains the list
        for x in range(0, rows):
            # 将元组里第x个列表水平排列
...
class ColorFollow:
    ...
    def get_position(self, rgb_img, hsv_msg, Function):
        ...
        # 筛选出位于两个数组之间的元素。
        # Filter out the elements located between two arrays
        color = cv.inRange(hsv_img, lowerb, upperb)
        # 设置非掩码检测部分全为黑色
        #  Set the non-mask detection part to all black
        color_mask[color == 0] = [0, 0, 0]
        # 将图像转为灰度图
        # Convert image to grayscale
        gray_img = cv.cvtColor(color_mask, cv.COLOR_RGB2GRAY)
        # 获取不同形状的结构元素
        #  Get different shapes of structural elements
    ...
    def Roi_hsv(self, img, Roi):
        ...
        :return: 图像和HSV的范围 例如：(0,0,90)(177,40,150)
                 The range of the image and HSV For example: (0,0,90)(177,40,150)
        '''
        H = [];S = [];V = []
