...
class StaticMenthod():
    ...
    def __str__(self):
        return self.count
control =StaticMenthod(1)
print(control.count)
#control_mode = StaticMenthod()
#control_mode.count = 0
...
class transbot_driver:
    def __init__(self):
        ...
        #self.bot = Transbot(arm_offset=bot_arm_offset)
        self.bot = Transbot(com="/dev/ttyAMA0")#bind a com.
        print("ok")
        # 弧度转角度
        # Radians turn angle
        self.RA2DE = 180 / pi
        ...
        self.sub_control_mode = rospy.Subscriber("/control_mode", Int32, self.control_mode_callback, queue_size=10)
    ...
    def sub_PWMServocallback(self, msg):
        # 云台控制，订阅者回调函数
        # PWMServo control, subscriber callback functionn
    ...
    def control_mode_callback(self, msg):
        control.count = msg.data + control.count
    ...
    def cmd_vel_callback(self, msg):
        ...
        if control.count == 0 or control.count%3 == 0:
              ...
              if angular_com > self.angular_max:
                  ...
                  angular_com = self.angular_min
              rospy.loginfo("cmd_vel: {}, cmd_ang: {}".format(velocity_com, angular_com))
              self.bot.set_car_motion(0.4*velocity_com, 0.3*angular_com)
        ...
        # Trolley motion control,velocity=[-0.45, 0.45], angular=[2, 2]
                ...
                self.bot.set_car_motion(0.3*velocity, 0.2*angular)
